...
COPY ./target/decision_trees_with_rules-1.0-SNAPSHOT.jar /var/lib/neo4j/plugins/
...
RUN apk update && apk add vim wget && \
    ...
    echo "dbms.security.procedures.unrestricted=com.maxdemarzi.*" > /var/lib/neo4j/conf/neo4j.conf
